...
func main() {
	http.HandleFunc("/cgi-bin/", func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			http.Error(w, "CGI stdin pipe failed", http.StatusInternalServerError)
			return
		}
		...
		go func() {
			...
			n, err := io.Copy(stdinPipe, io.NopCloser(bytes.NewReader(bodyBytes)))
			log.Printf("CGI stdin copy [%s]: wrote %d bytes, err=%v", toolPath, n, err)
		}()
